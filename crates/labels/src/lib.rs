// TODO: send through spellcheck

pub const DRIVER_OPMODE_TITLE: &str = "Scaling driver operation mode";
pub const DRIVER_OPMODE_TT: &str = "The operation mode of the CPU frequency scaling driver. Passive will give control of requency scaling to the CPUFreq kernel driver. In active mode the CPU manages frequencies.";
...
pub const EPP_UNAVAILABLE_TT: &str =
    "EPP/EPB is only available on Intel Core i's 2nd gen or newer and AMD Zen 2 or newer.";

pub const GOV_TITLE: &str = "Scaling governor";
pub const GOV_TT: &str = "Parameter of the scaling driver that configures it's behaviour. The available governors differ depending on whether the scaling driver is in passive/active operation mode.";
...
pub const PERF_PCT_UNAVAILABLE_TT: &str = "Configuring minimum/maximum p-state percentage is only only available in Intel Core i's 2nd gen or newer.";
...
pub const HWP_DYN_BOOST_UNAVAILABLE_TT: &str =
    "HWP Dyanmic Boost is only available on Intel Core i's 6th gen or newer.";
...
pub const VM_WR_TT: &str = "In modern operating systems data is first written to memory and then written to disk in predfined intervals for performance and power saving reasons. This option defines the interval for memory flushing to disk.";
...
pub const DIS_ETH_TT: &str = "Some tools such as powertop report that the ethernet port uses 2-3 watts when not connected. While these values may be incorrect, disabling ethernet completly if not in use is common in users looking for best power savings in their devices.";

pub const IWLWIFI_POWERSAVING_TITLE: &str = "Enable WiFi driver powersaving";
pub const IWLWIFI_POWERSAVING_TT: &str =
    "Configres the power_save parameter in the iwlwifi network driver.";

pub const UAPSD_TITLE: &str = "Enable U-APSD";
pub const UAPSD_TT: &str = "U-APSD stands fo Unscheduled Automatic Power Save Delivery and is a part of the 802.11e standard. It allows the Network card to go into standby mode when no packets are being received. May cause performance loss.";
...
pub const WIFI_POWERSCHEME_TT: &str = "Configures the power_scheme parameter in the iwlmvm Kernel driver. 1 - means allways on, 2 - balanced and 3 - lower power. If the the iwldvm driver is being used instead, the force_cam=0 parameter will be set if this value is equal to 3.";
